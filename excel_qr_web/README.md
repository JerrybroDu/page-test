...
- 页面**自适应屏幕宽度**（手机 / 平板 / 电脑），下拉面板始终在视口内不越界
- 生成完成后**弹框确认是否下载**：显示文件名与保存位置，弹框期间背景页面锁定不可滚动；点击「下载文件」后出现**转圈保存提示**（Chrome / Edge 可自选保存位置，其余浏览器下载到默认「下载」目录），不会自动下载
- 下载文件名自动带**年月日**：`原文件名_二维码_年月日.xlsx`（如 `名单_二维码_20260823.xlsx`）
...
│   ├── responsive_test.html # 多端自适应：主题面板不越界、预览无横向滚动（375/820/1200px）
│   ├── download_confirm_test.html # 生成后弹框确认下载：展示文件名/保存位置、取消不下载
...
网页版没有服务器，无法像桌面版那样“另存到任意文件夹”，因此结果以**浏览器下载**方式保存（文件名自动为 `原文件名_二维码_年月日.xlsx`，如 `名单_二维码_20260823.xlsx`）。
...
| 宏（.xlsm） | 保留 | 转存后不保留宏 |
| 另存路径 | 可自选文件夹 | 生成后弹框确认（含转圈提示）；Chrome/Edge 可自选保存位置，其余浏览器下载到默认「下载」目录（自动命名：`原文件名_二维码_年月日.xlsx`） |
| 文件体积上限 | 无 | 50MB（纯前端解析） |
...
# test/responsive_test.html           多端自适应（375/820/1200px 下主题面板不越界、预览无横向滚动）
# test/download_confirm_test.html     下载确认弹框（展示文件名/保存位置、取消不下载、确认才下载）
```
