...
(async () => {
  ...
  for (const item of rows) {
    ...
    ws.addImage(imgId, { tl: { col: colQr - 1, row: item.r - 1 }, ext: { width: entry.size, height: entry.size } });
    // 自动调整行高列宽（与 js/app.js 相同公式）
    const targetH = entry.size * 0.75 + 4;
    const row = ws.getRow(item.r);
    if (!row.height || row.height < targetH) row.height = targetH;
    const col = ws.getColumn(colQr);
    const targetW = entry.size / 7 + 2;
    if (!col.width || col.width < targetW) col.width = targetW;
  }
  ...
  assert(first.col === 3 && first.row === 1, '首张二维码锚定在 D2（col=' + first.col + ',row=' + first.row + '）');
